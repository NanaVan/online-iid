...
class PLS(object):
    ...
    #    '''
    #    beta_ratio = 0.01
    #    L, beta = len(self.data), 0.5*np.ones_like(self.data) 
    #    D = sparse.diags([1,-2,1], [0,-1,-2], shape=(L, L-2))
    #    D = l * D.dot(D.transpose())
    #    w, z = np.ones(L), self.data.copy()
    #    warnings.filterwarnings("ignore")
    ...
    #            if np.sqrt(np.sum((z - zt)**2) / np.sum(z**2)) < ratio: break
    #            z = zt
    #        if np.allclose(beta, np.where(self.data-z>0, 1-beta_ratio, beta_ratio)): break
    #        beta = np.where(self.data-z>0, 1-beta_ratio, beta_ratio)
    #    return z
